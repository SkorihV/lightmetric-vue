...
// Не буду особо комментировать файлик, нужен рефакторинг больших методов и их описание.
// Предлагаю посмотреть как будет выглядить с нормальным форматирование.
export default {
  methods: {
    ...mapActions(["SET_COMPUTED_VALUE"]),
    processingFormula() {
      let params = this.getArrayParams(this.data.type_id);
      let resultValueString = "";
      if (params.length) {
        resultValueString = this.generateResultOnFormulaInCell(
          this.data,
          params
        );
        ...
        try {
          ...
          console.log(
            `Ошибка при расчете формулы ${this.formula} в ячейке ${
              this.data.planed_at
            } строки ${this.categoryNameById(this.data.type_id)} с ID = ${
              this.data.type_id
            }`
          );
        }
    ...
    getArrayParams(metricId) {
      // Точно нужно описать комментами
    ...
    generateResultOnFormulaInCell(cell, params) {
      if (cell.value == null && !params) {
        ...
        cell.value !== null &&
        cell.value.toString().match("^[0-9]+:[0-5][0-9](:[0-5][0-9])?")
      ) {
        return false;
      }
    ...
    getFormulaCell(param, cell) {
      // FIXME: нужен рефакторинг, слишком сложно для простой функции
  ...
  computed: {
    ...mapGetters(["allMetrics", "metricForId", "categoryNameById"]),
  },
};
